...
```
C:\trade-desk\CLAUDE.md          ← 假設放了「開發指示」
└── agents\chart\                ← CHART 的 cwd
```

CHART 啟動時會載入 `agents/chart/CLAUDE.md` **以及** `agents/CLAUDE.md` **以及** `C:\trade-desk\CLAUDE.md`。
...
```powershell
cd C:\trade-desk
git worktree add C:\trade-desk-work\feat-xxx -b feat/xxx
cd C:\trade-desk-work\feat-xxx\dev
claude
```
這樣運行中的 `C:\trade-desk` 完全不受影響。FORGE 修 bug 時走的也是這條路徑。
